...
export default function TomarPedido({ ordenAEditar, onTerminarEdicion, user: propUser, cajaAbiertaGlobal }) {
  ...
  // SOLUCIÓN AL ERROR DE LA CAJA CERRADA: Restaurado el estado local
  ...
  // EFECTO PARA VALIDAR CAJA ABIERTA LOCALMENTE EN TIEMPO REAL
  useEffect(() => {
    if (!user) return;
  ...
  const enviarCocina = async () => {
    ...
    try {
        ...
        // Registrar anulación en la base de datos si el pedido ESTABA pagado previamente
  ...
  return (
    <div className="flex h-full bg-slate-100 overflow-hidden font-sans text-gray-800 relative main-app-container">
      {/* NOTIFICACIÓN FLOTANTE MEJORADA - BOTTOM-4 */}
      ...
      <aside className="w-[400px] h-full bg-white shadow-xl flex flex-col z-20 border-r border-gray-200 flex-shrink-0 no-print">
        <div className="p-3 border-b border-gray-100 bg-gray-50 flex-shrink-0 space-y-2">
           ...
           {!cajaAbierta && <div className="bg-amber-50 border border-amber-200 p-2 rounded-xl text-center shadow-inner"><p className="text-[10px] font-black text-amber-600 uppercase m-0 tracking-tighter animate-pulse">⚠️ Debe abrir caja para agendar pedidos</p></div>}

...
// Función para obtener fecha YYYY-MM-DD (UTC)
const getLocalDate = () => new Date().toISOString().split('T')[0];

export default function App({ user: userProp }) {
    ...
    // Carga de Auth si no viene por prop
    useEffect(() => {
        if (!userProp) {
    ...
    const handleExportarPDF = () => {
        ...
        doc.setFontSize(14); doc.text("REPORTE DE CAJA DETALLADO", 105, 22, { align: 'center' });
        doc.setFontSize(10); doc.setFont("helvetica", "normal");
        doc.text(`Desde: ${fechaInicioCaja} | Emisión: ${hoyString}`, 105, 28, { align: 'center' });

        // 1. RESUMEN
        doc.setFont("helvetica", "bold"); doc.text("1. RESUMEN FINANCIERO", 14, 40);
        doc.autoTable({
            startY: 42,
            head: [['Concepto', 'Descripción', 'Monto']],
            body: [
                ['Ventas Netas', 'Productos pagados y entregados', formatoPeso(totalVentasNetas)],
                ['Envíos', 'Recaudado para delivery', formatoPeso(totalEnvios)],
                ['Gastos', 'Egresos del turno', formatoPeso(totalGastos)],
                ['GANANCIA REAL', 'Utilidad final', formatoPeso(gananciaReal)]
            ],
        ...
        doc.autoTable({
            ...
            head: [['Concepto', 'Monto']],
            body: [
                ['Fondo de Apertura', formatoPeso(montoApertura)],
                ['Ventas Efectivo (+)', formatoPeso(efectivo)],
                ['Gastos Efectivo (-)', `-${formatoPeso(totalGastos)}`],
                ['TOTAL EN CAJÓN', formatoPeso(efectivoEnCajon)]
            ],
            theme: 'striped', headStyles: { fillColor: [30, 41, 59] }
        });

        // 3. DESGLOSE PAGOS (TABLA SOLICITADA)
        doc.setFont("helvetica", "bold"); doc.text("3. DESGLOSE POR TIPO DE PAGO", 14, doc.lastAutoTable.finalY + 12);
        doc.autoTable({
            startY: doc.lastAutoTable.finalY + 15,
            head: [['Medio de Pago', 'Cantidad', 'Monto Total']],
            body: [
                ['Efectivo', conteoPagos.Efectivo, formatoPeso(efectivo)],
                ['Tarjeta', conteoPagos.Tarjeta, formatoPeso(tarjeta)],
                ['Transferencia', conteoPagos.Transferencia, formatoPeso(transferencia)],
        ...
        doc.setFont("helvetica", "bold"); doc.text("4. DETALLE DE VENTAS", 14, doc.lastAutoTable.finalY + 12);
        doc.autoTable({
            startY: doc.lastAutoTable.finalY + 15,
            head: [['Nº', 'Hora', 'Cliente', 'Productos', 'Total']],
        ...
        doc.save(`Cierre_Isakari_${fechaInicioCaja}.pdf`);
    };

    if (cargando) return <div className="h-full flex items-center justify-center font-black uppercase text-slate-400 animate-pulse">Sincronizando...</div>;
        ...
        <div className="flex flex-col h-full bg-slate-100 p-6 font-sans overflow-hidden text-gray-800">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                        {idCajaAbierta ? `Abierta desde: ${fechaInicioCaja}` : "Turno Cerrado"}
                    </span>
                </div>
                <div className="flex gap-2">
                    <button onClick={handleExportarPDF} className="bg-white border-2 border-slate-200 text-red-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-red-50 transition-all">REPORTE PDF</button>
                    <button onClick={handleCerrarCaja} disabled={!idCajaAbierta} className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg transition-all ${idCajaAbierta ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-400'}`}>CERRAR TURNO</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-emerald-600 p-5 rounded-[2rem] shadow-xl text-white border border-emerald-700">
                    <span className="text-[9px] font-black uppercase opacity-70">Ventas Netas</span>
                    <div className="text-2xl font-black tracking-tighter">{formatoPeso(totalVentasNetas)}</div>
                </div>
                <div className="bg-orange-600 p-5 rounded-[2rem] shadow-xl text-white border border-orange-700">
                    <span className="text-[9px] font-black uppercase opacity-70">Envíos</span>
                    <div className="text-2xl font-black tracking-tighter">{formatoPeso(totalEnvios)}</div>
                </div>
                <div className="bg-rose-600 p-5 rounded-[2rem] shadow-xl text-white border border-rose-700">
                    <span className="text-[9px] font-black uppercase opacity-70">Gastos</span>
                    <div className="text-2xl font-black tracking-tighter">{formatoPeso(totalGastos)}</div>
                </div>
                <div className="bg-slate-900 p-5 rounded-[2rem] shadow-2xl text-white border border-slate-800">
                    <span className="text-[9px] font-black uppercase opacity-70">Utilidad</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 overflow-hidden">
                <div className="bg-white rounded-[2rem] border-2 border-slate-100 flex flex-col overflow-hidden shadow-sm">
